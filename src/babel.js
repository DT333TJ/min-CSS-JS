...
var cssHref = location.href.indexOf('taikang') !== -1 ? 'http://10.132.23.192/starlinkdashboard' : 'http://10.154.31.90/starlinkdashboard';
var productionUrl = 'http://oa.taikang.com/starlinkdashboard';
var uatUrl = 'http://oa.tkuat.com/starlinkdashboard'; // const uatUrl = 'http://localhost:7552/starlinkdashboard'
...
var StarlinkDashBoard = /*#__PURE__*/function () {
  ...
  _createClass(StarlinkDashBoard, [{
    ...
    value: function openDrawer(urlObj) {
      ...
      var src = (location.href.indexOf('taikang') !== -1 ? productionUrl : uatUrl) + "/".concat(systemCode, "/dashboard?").concat(paramsString) + "currentDashboard=".concat(systemCode, "&oriDashboard=").concat(systemCode, "&storageID=").concat(new Date().valueOf(), "&storageIndex=0"); // 设置iframe url
    ...
    value: function onMessageHandle(event) {
      ...
      if (event.origin === data.targetOrigin && data.postMessageName === 'starlinkdashboard') {
        ...
        if (data.postMessageType === 'fallback' && data.params) {
          var _url = data.params.url; // 回退看板

          this.initIframeElementAndUrl(_url);
        } // 接收到切换其他系统看板操作


        if (data.postMessageType === 'jumpDashboard' && data.params) {
          var _url2 = data.params.url; // 展示看板

          this.initIframeElementAndUrl(_url2);
        } // 接收到同源看板跳转外系统页面的操作


        if (data.postMessageType === 'jumpOtherSystemPage' && data.params) {
          var _url3 = data.params.url; // 跳转到空白页面则不需要关闭看板

          this.options.commonOriOtherPageJump !== '_blank' && this.closeDrawer();
          window.open(_url3, this.options.commonOriOtherPageJump);
        }
      }
    }
    /**
...
exports.default = StarlinkDashBoard;